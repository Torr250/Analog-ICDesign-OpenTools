...
T {tcleval(VDSatP: [to_eng [xschem raw value vdsatp 0]])} 60 270 0 0 0.3 0.3 {floater=1}
T {tcleval(VDSP: [to_eng [xschem raw value vdsp 0]])} 60 300 0 0 0.3 0.3 {floater=1}
T {tcleval(gmIDP: [to_eng [xschem raw value gmidp 0]])} 60 240 0 0 0.3 0.3 {floater=1}
...
N 50 -140 90 -140 {lab=VBP}
N 130 -110 130 -50 {lab=out}
N 430 -210 430 -180 {lab=VBP}
...
.control
  ...
  let gdsp = @m.xm2.m0[gds]
  let id = @m.xm1.m0[id]
  let vdsatn = @m.xm1.m0[vdsat]
  let vdsatp = @m.xm2.m0[vdsat]
  let vdsn = @m.xm1.m0[vds]
  let vdsp = @m.xm2.m0[vds]
  let gmIDn = gmn/id
  let gmIDp = gmp/id
  let Aintr = gmn/(gdsn+gdsp)
...
C {symbols/pfet_03v3.sym} 110 -140 0 0 {name=M2
L=0.58u
W=14.6u
nf=1
m=20
...
C {vsource.sym} 430 -150 0 0 {name=V3 value='VDD-0.945' savecurrent=false}
C {gnd.sym} 430 -120 0 0 {name=l8 lab=0}
C {lab_wire.sym} 50 -140 0 0 {name=p3 sig_type=std_logic lab=VBP}
C {lab_wire.sym} 430 -210 0 0 {name=p4 sig_type=std_logic lab=VBP}
